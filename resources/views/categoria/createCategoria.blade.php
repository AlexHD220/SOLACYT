...
<x-plantilla-body>
    ...
    <form action="/categoria" method="post"> <!--la diagonal me envia al principio de la url "solacyt.test/"-->
        ...
        <input type="text" id="nombre" name="nombre" placeholder="Categoría" required value = "{{ old('nombre') }}"><br><br> <!--value = "{{old('name')}}"-->

        <label for="descripcion" style="margin-bottom: 5px;"><b> Descripción: </b></label><br>
        <textarea id="descripcion" name="descripcion" rows="4" style="resize: none; width: 400px;" required>{{ old('descripcion') }}</textarea><br><br>
